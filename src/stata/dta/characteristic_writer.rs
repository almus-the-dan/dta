...
impl<W: Write> CharacteristicWriter<W> {
    ...
    fn write_xml_entry(&mut self, characteristic: &Characteristic) -> Result<()> {
        let byte_order = self.header.byte_order();
        let encoded_value = self.encode_value(characteristic.value())?;
        let payload_len = self.payload_length(encoded_value.len())?;
    ...
    /// Encodes the value string via the active encoding. Returns a
    /// `Cow<[u8]>` tied to the input — borrowed (zero-allocation)
    /// whenever the active encoding is a pass-through (UTF-8 source and
    /// UTF-8 target for V118+) and owned otherwise. The borrow
    /// lifetime is the input string's, not `self`'s, so the caller
    /// can hold the result across later `&mut self.state`
    /// operations.
    fn encode_value<'a>(&self, value: &'a str) -> Result<Cow<'a, [u8]>> {
        let position = self.state.position();
        let (encoded, _, had_unmappable) = self.state.encoding().encode(value);
        if had_unmappable {
            return Err(DtaError::format(
                Section::Characteristics,
                position,
    ...
    /// Total entry payload length: `2 × variable_name_len +
    /// encoded_value_len`. Errors if the total doesn't fit `u32`
    /// (the length field is at most 4 bytes in every release).
    fn payload_length(&self, encoded_value_len: usize) -> Result<u32> {
        let variable_name_len = self.header.release().variable_name_len();
        let total = variable_name_len
            .checked_mul(2)
            .and_then(|names_len| names_len.checked_add(encoded_value_len))
            .ok_or_else(|| self.payload_overflow_error(encoded_value_len))?;
        u32::try_from(total).map_err(|_| self.payload_overflow_error(encoded_value_len))
    }

    fn payload_overflow_error(&self, encoded_value_len: usize) -> DtaError {
        DtaError::format(
            Section::Characteristics,
            self.state.position(),
