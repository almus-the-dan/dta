...
impl<W> LongStringWriter<W> {
    ...
    /// [`LongStringTable::iter`](LongStringTable::iter)
...
impl<W: Write + Seek> LongStringWriter<W> {
    ...
    pub fn write_long_string_table(&mut self, table: &LongStringTable) -> Result<()> {
        // `LongStringTable::iter` captures the writer's encoding
        // synchronously and returns an iterator that borrows only
        // the table — so we're free to re-borrow `self` as `&mut`
        // inside the loop body.
        for long_string in table.iter(&*self) {
