...
- `AsyncRecordReader::seek_to_record(index)` — the async equivalent, available under the existing `tokio` feature wherever `R: AsyncRead + AsyncSeek + Unpin`. The bounds-check, offset arithmetic, and overflow handling are factored into a shared `compute_record_seek_target` helper used by both sync and async paths, so the two implementations stay in lockstep.
...
- New top-level module `dta::stata::dct` with a streaming reader for Stata dictionary (`.dct`) files. DCT files describe the schema of fixed-width or free-format plain-text data and are common when importing data into Stata from external systems. The new module sits alongside `dta::stata::dta` and shares the format-agnostic Stata-domain types (`MissingValue`, `StataByte`/`Int`/`Long`/`Float`/`Double`).
  ...
  - `DctReaderOptions::record_warnings(bool)` opts out of warning collection entirely. When disabled, the reader skips constructing `DctWarning` values and `warnings()` always returns an empty slice — useful in tight loops where the diagnostic structs would otherwise dominate per-record cost.
...
- `From` and `TryFrom` impls between the five Stata numeric types — `StataByte`, `StataInt`, `StataLong`, `StataFloat`, `StataDouble` — mirroring the `From` / `TryFrom` lattice that Rust provides for the underlying primitives. `From` covers every lossless widening (e.g., `StataByte → StataInt`, `StataInt → StataDouble`, `StataFloat → StataDouble`); `TryFrom` covers integer narrowing (e.g., `StataLong → StataInt`) with `Error = std::num::TryFromIntError`. `Missing(MissingValue)` translates directly through both directions so the `.` / `.a`–`.z` tag is preserved across widths. Notably, there is no `StataLong → StataFloat` impl — symmetric to Rust omitting `From<i32> for f32` because not every `i32` is exactly representable in `f32`.
